...
`{"type":"connect",
  ...
  generateInstanceActions(
    translationService: TranslationService,
      ...
      menuItems = [
        ...
        },
        {
          ...
          "children": [
            {
            "text": "Disabled - Won't Appear",
            "icon": 'icon-person',
            "action": (closeContextMenu: () => void) => {
              console.log("Disabled - Won't Appear");
              closeContextMenu()
            }
            },
            {
        ...
        {
          "text": "Don't Close Menu",
          "icon": 'icon-settings',
          "action": (closeContextMenu: () => void) => {
            console.log("Don't Close Menu");
          },
          "children": [
            {
            ...
            {
              "text": "Keyboard Shortcut + Close Menu",
              "icon": 'icon-person',
              "shortcut": 'F6',
              "action": (closeContextMenu: () => void) => {
                console.log("Keyboard Shortcut + Close Menu");
                closeContextMenu()
              }
            }
          ]
    ...
    onRightClick(event: MouseEvent): boolean {
      let menuItems: ContextMenuItem[] = this.generateInstanceActions(this.translation);
      let appPosition = document.getElementsByClassName("test-panel-container")[0].getBoundingClientRect()
      if (this.windowActions) {
        this.windowActions.spawnContextMenu(event.clientX - appPosition.left, event.clientY - appPosition.top, menuItems);
      }
      return false;
    }
}
